    ...
    foreach ($combined_data as $item) {
        ...
        switch ($type) {
            case 'view':
                ...
                break;
            case 'activity':
                $output .= $day_of_week. ', ' . $date . ' ' . $time . ' - ' . ucfirst(esc_html($data['action'])) . ': <a href="' . get_permalink($data['post_id']) . '">' . esc_html($data['post_title']) . '</a> <small>(' . esc_html($data['post_type']) . ', ' . esc_html($data['post_status']) . ')</small>';
                break;
            case 'profile_change':
